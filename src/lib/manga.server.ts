...
/** Agnes AI image endpoint (OpenAI-compatible images API). */
const AGNES_URL =
  process.env["AGNES_IMAGE_URL"]?.trim() || "https://apihub.agnes-ai.com/v1/images/generations";
/** The one image model this app renders with. */
const AGNES_MODEL = process.env["AGNES_IMAGE_MODEL"]?.trim() || "agnes-image-2.1-flash";
/** Landscape panel size sent to the renderer. */
const IMAGE_SIZE = process.env["AGNES_IMAGE_SIZE"]?.trim() || "1344x768";
 ...
 * this exact block is added at the final image request for every panel.
 * Stated entirely positively: naming unwanted media such as photography or
...
/**
 * Minimal medium tag. Just enough to keep the renderer off its photographic
 * default — and, critically, without ever naming faces or eyes as things to
 * draw.
 */
const STYLE_LEAD =
...
/**
 * Single-image guard. Stated positively that there is exactly ONE continuous
 * illustration filling the frame — otherwise "manga" reads as permission to
 * draw a whole multi-panel page. Everything here describes the wanted result:
 * naming "speech bubbles" or "collage" — even to forbid them — puts those very
 * tokens into the picture, which is what kept producing pages with balloons.
 */
const SINGLE_FRAME_GUARD =
  "exactly one full-bleed manga illustration filling the whole image, one continuous scene edge to edge, " +
  "smooth clean artwork surface";
...
/** Renders one panel with Agnes AI (`agnes-image-2.1-flash`), with retries. Always 16:9. */
export async function generateImage(
  ...
  for (let attempt = 0; attempt < Math.max(1, attempts); attempt++) {
    ...
    assertActive();
    // One Agnes key renders a limited number of images at a time: this waits
    // for capacity so the account is never flooded.
      ...
      try {
        const res = await fetch(AGNES_URL, {
          ...
          body: JSON.stringify({
            ...
            n: 1,
            size: IMAGE_SIZE,
            // A fresh seed each attempt, so a blank frame is never re-rolled
            // identically. Agnes only accepts 0-999, so the caller's seed is
            // folded into that range.
            seed: Math.abs(Math.trunc(seed) + attempt * 977) % 1000,
          }),
        });
        if (res.ok) {
          ...
          const out = first?.url?.trim()
            ? first.url.trim()
            : first?.b64_json?.trim()
              ? `data:image/png;base64,${first.b64_json.trim()}`
              : "";
          if (out) {
            if (out.startsWith("data:") || (await isRealImage(out))) return out;
            lastErr = "blank image rejected";
          } else {
            lastErr = json.error?.message ?? "no image returned";
          }
        } else {
